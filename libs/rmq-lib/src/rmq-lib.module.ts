...
export class RmqLibModule
  extends createConfigurableDynamicRootModule<RmqLibModule, RabbitMQConfig>(
    RABBIT_CONFIG_TOKEN,
    {
      providers: [
        {
          ...
          useFactory: async (
            ...
            await RmqLibModule.AmqpConnectionFactory(config);
            return RmqLibModule.connectionManager;
          },
...
{
  private readonly logger = new Logger(RmqLibModule.name);
  ...
  static async AmqpConnectionFactory(config: RabbitMQConfig) {
    ...
    const logger = config.logger || new Logger(RmqLibModule.name);
      ...
      providers: [
        {
          ...
          useFactory: async (): Promise<AmqpConnection> => {
            return RmqLibModule.AmqpConnectionFactory(config);
          },
        },
        RabbitRpcParamsFactory,
      ],
  ...
  async onApplicationShutdown() {
    ...
    RmqLibModule.bootstrapped = false;
  }

  // eslint-disable-next-line sonarjs/cognitive-complexity
  public async onApplicationBootstrap() {
    if (RmqLibModule.bootstrapped) {
      return;
    }
    RmqLibModule.bootstrapped = true;
