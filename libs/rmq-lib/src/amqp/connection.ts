...
export type RpcResponseHandler<T, U> = (
  msg: T | undefined,
  rawMessage?: ConsumeMessage,
  headers?: any,
) => Promise<RpcResponse<U>>;
...
export class AmqpConnection {
  ...
  private _handlers: Record<
    string,
    SubscriberHandler<unknown> | RpcResponseHandler<unknown, unknown>
  > = {};
  ...
  private getHandlerId(
    options: Pick<MessageHandlerOptions, 'exchange' | 'routingKey' | 'queue'>,
  ) {
    return `${options.exchange}::${options.routingKey}::${options.queue}`;
  }
  ...
  private async setupSubscriberChannel<T>(
    ...
    const { consumerTag }: { consumerTag: ConsumerTag } = await channel.consume(
      ...
      async (msg) => {
        try {
          ...
          //for some reason no ability to requeue
          // const handlerId = this.getHandlerId({ ...msg.fields, queue });
    ...
    const handlerId = this.getHandlerId(msgOptions);
  ...
  public async setupRpcChannel<T, U>(
    ...
    const { consumerTag }: { consumerTag: ConsumerTag } = await channel.consume(
      ...
      async (msg) => {
        try {
          ...
          //for some reson no ability to requeue
          // const handlerId = this.getHandlerId({ ...msg.fields, queue });
          // const handler = this._handlers[handlerId];
    ...
    const handlerId = this.getHandlerId(rpcOptions);
    this._handlers[handlerId] = handler;
